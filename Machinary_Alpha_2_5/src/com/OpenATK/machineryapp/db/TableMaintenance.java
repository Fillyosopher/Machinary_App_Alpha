	...
	// Database table
		...
		public static MachineTypeList cursorToMachineTypeList(Cursor cursor){
			if(cursor != null){
				...
				Integer order = cursor.getInt(cursor.getColumnIndex(TableMaintenance.COL_ORDER));
				Date dateOrderChanged = DatabaseHelper.stringToDateUTC(cursor.getString(cursor.getColumnIndex(TableMaintenance.COL_ORDER_CHANGED)));
		...
		public static MachineTypeList FindMachineTypeListByName(DatabaseHelper dbHelper, String name) {
			...
			if (name != null) {
				...
				Cursor cursor = database.query(TableMaintenance.TABLE_NAME,
						TableMaintenance.COLUMNS, where, null, null, null, null);
				if (cursor.moveToFirst()) {
					theMachineTypeList = TableMaintenance.cursorToMachineTypeList(cursor);
				}
				cursor.close();
				database.close();
				dbHelper.close();
		...
		public static MachineTypeList FindMachineTypeListById(DatabaseHelper dbHelper, Integer id) {
			...
			if (id != null) {
				...
				Cursor cursor = database.query(TableMaintenance.TABLE_NAME,TableMaintenance.COLUMNS, where, null, null, null, null);
				if (cursor.moveToFirst()) {
					theMachineTypeList = TableMaintenance.cursorToMachineTypeList(cursor);
				}
				cursor.close();
		...
		public static boolean updateMachineTypeList(DatabaseHelper dbHelper, MachineTypeList list){
			...
			if(list.getRemote_id() != null) values.put(TableMaintenance.COL_REMOTE_ID, list.getRemote_id());

			if(list.getDateNameChanged() != null) values.put(TableMaintenance.COL_NAME_CHANGED, DatabaseHelper.dateToStringUTC(list.getDateNameChanged()));
			if(list.getName() != null) values.put(TableMaintenance.COL_NAME, list.getName());

			if(list.getOrder() != null) values.put(TableMaintenance.COL_ORDER, list.getOrder());
			if(list.getDateOrderChanged() != null) values.put(TableMaintenance.COL_ORDER_CHANGED, DatabaseHelper.dateToStringUTC(list.getDateOrderChanged()));

			if(list.getDeleted() != null) values.put(TableMaintenance.COL_DELETED, (list.getDeleted() == false ? 0 : 1));
			if(list.getDateDeleted() != null) values.put(TableMaintenance.COL_DELETED_CHANGED, DatabaseHelper.dateToStringUTC(list.getDateDeleted()));


			if(list.getId() == null && (list.getRemote_id() == null || list.getRemote_id().length() == 0)) {
				//INSERT This is a new worker, has no id's
				int id = (int) database.insert(TableMaintenance.TABLE_NAME, null, values);
				list.setId(id);
				ret = true;
			} else {
				//UPDATE
				...
				String where;
				if(list.getId() != null){
					where = TableMaintenance.COL_ID + " = " + Integer.toString(list.getId());
				} else {
					where = TableMaintenance.COL_REMOTE_ID + " = '" + list.getRemote_id() + "'";
				}
				database.update(TableMaintenance.TABLE_NAME, values, where, null);
				ret = true;
			}
